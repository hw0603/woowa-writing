...
### URL의 구성 요소
우리는 URL을 통해 특정 사이트에 접속하게 된다. URL은 하나의 문자열이지만, 여러 부분으로 구성되어 있다.

![image](https://github.com/user-attachments/assets/973ccf5c-0d2e-4ecc-ae48-1a3df4231478)
...
CORS의 `C` 가 cross를 의미하는 만큼, 여러 URL이 있을 때 어떤 URL이 같은 Origin이고, 어떤 URL이 다른 Origin인지 판단할 수 있어야 한다.
...
### SOP가 필요했던 이유
인터넷 환경에는 어디에나 위험이 도사리고 있다. SOP를 통해 같은 Origin에서만 리소스를 공유할 수 있도록 제한하지 않는다면 악의적인 사용자가 XSS(Cross-Site Scripting)나 CSRF(Cross-Site Request Forgery) 공격을 통해 정상적인 사용자의 정보를 탈취할 수 있다.  
...
이와 같은 비정상적인 경우를 미연에 방지하기 위해, 웹 보안의 기본적인 방어선으로서 SOP가 등장하게 된 것이다.
...
### CORS의 유효성
CORS 허용 Origin의 리스트는 서버에서 관리하지만, 이는 서버를 보호하기 위함이 아니다. 악의적인 비 정상 클라이언트가 웹 상에 배포되어 정상적인 사용자가 해당 클라이언트를 통해 서버에 요청을 보낼 때, 사용자의 브라우저가 해당 요청을 미리 거부하도록 하여 보안 수준을 유지한다.
...
하지만, 프론트엔드와 백엔드 서버가 분리되어 CORS 설정이 필요한 대부분의 경우 데이터 교환은 JSON으로 이루어지기 때문에, `Content-Type`은 `application/json`인 경우가 많으며 사용자 식별을 위해 쿠키를 전송해야 하는 경우 `Cookie` 헤더를 사용하기 때문에 단순 요청을 만족하는 요청은 그렇게 많지 않다.

### 사전 요청 (Preflight Request)
서버는 CORS 위반 여부와 관계없이 요청이 수신되면 처리 후 응답을 전송한다. 단순 요청에서는 응답을 받은 브라우저가 `Access-Control-*` 응답 헤더와 비교 후 파기 여부를 결정한다.  
이때, `GET`이나 `HEAD`와 같은 조회성 메서드들은 큰 문제가 되지 않지만, `POST`나 `DELETE`같이 상태를 변경시키는 메서드들은 서버에 부수 효과(Side Effect)를 야기할 수 있다. 예를 들어, 사용자의 특정 정보를 삭제하는 `DELETE` 요청을 다른 Origin에서 전송했을 때, 브라우저는 CORS 정책에 따라 서버의 응답을 파기하지만, 서버에서는 정보가 이미 삭제된 상태로 남아 있을 것이다.
...
`Access-Control-Request-Method`와 `Origin` 헤더를 사용하여 `HTTP OPTION` 메서드를 통해 서버로 요청을 전송한다. 선택적으로 `Access-Control-Request-Headers` 헤더를 추가로 사용할 수도 있다.

간단히 말해 실제 요청하고자 하는 메서드와 헤더 정보를 서버에게 미리 제공하고, 서버의 CORS 정책 위반 여부를 미리 검증하고, CORS 위반 요청의 경우 본 요청 자체를 보내지 않음으로써 서버를 보호하는 역할을 한다.

#### 사전 요청의 오버헤드
사전 요청은 본 요청을 전송하기 전 미리 요청을 보내 CORS 위배 여부를 확인하는 요청이라고 했다. 이는 결국 하나의 요청을 수행하기 위해 매 번 두 개의 요청을 전송해야 한다는 의미가 되고, 애플리케이션 성능에 나쁜 영향을 미친다.

이러한 문제를 해결하기 위해 서버는 `Access-Control-Max-Age` 응답 헤더를 통해 브라우저에서 사전 요청의 결과를 캐싱하는 기간을 초 단위로 설정할 수 있다.  
브라우저는 사전 요청을 진행할 때 마다 캐시를 확인하여 캐시에 유효한 값이 존재하는지 확인하고, 캐싱되어 있지 않은 경우에만 실제 사전 요청을 전송하여 오버헤드를 최소화할 수 있다.
...
- `same-origin`: 동일 Origin간의 요청에만 자격 증명을 포함하여 전송
...
> [!WARNING]  
> 로그인 API 등 요청에는 사용자 정보가 필요하지 않지만, 응답을 통해 쿠키를 설정해야 하는 경우에도 `credentials` 옵션을 명시해 주어야 한다.  
> `withCrednetials`가 `true`로 설정되지 않은 `XMLHttpRequest` 는 쿠키를 설정할 수 없으며, 서버의 응답에 `Set-Cookie` 헤더가 존재하더라도 브라우저는 이를 무시한다.

### CORS는 브라우저 스펙이다
![image](https://blog.sucuri.net/wp-content/uploads/2022/11/22-sucuri-CORS-Security-Header-Blog-Image-1.png)  
위 시나리오를 통해 이미 눈치 챈 독자도 있겠지만, CORS 위배 여부의 판단은 **서버에서 제공한 정보를 기반으로 브라우저가 수행**한다. 서버는 단순히 허용 정책을 설정하는 역할을 할 뿐이며, 서버로부터 정상적인 응답이 수신되더라도 브라우저가 Origin이 다르다고 판단하면 해당 응답을 파기한다.

> [!NOTE]  
> 실제로 크로미움 기반 브라우저는 실행 시 `flag`를 추가하여 CORS 검사를 비활성화 할 수 있는 기능을 제공하고 있으며, 상황에 따라 임시로 다른 Origin간의 통신이 필요할 때 유용하게 사용할 수 있다.
>
> CORS는 브라우저 스펙이므로 서버-서버 간 통신에는 적용되지 않는다. 따라서 프록시 서버를 거쳐 통신하는 방법을 통해 CORS에러를 해결할 수도 있다. 이에 대한 자세한 설명은 잠시 후 알아보도록 하자.
...
### 로컬 프록시 서버 활용
CORS Error를 가장 많이 접할 수 있는 경우는 프론트엔드 로컬 개발환경에서 원격에 배포된 백엔드 서버에 요청을 전송하는 경우일 것이다.

CORS는 브라우저에서 관리하는 정책이므로, 서버와 서버간의 통신에는 적용되지 않는다.  
이 점을 활용하여 로컬 환경에 프록시 서버를 구축하고, 클라이언트는 로컬 프록시로 요청을 보낸 다음 프록시에서 해당 요청을 실제 서버로 포워딩해주는 작업을 통해 CORS 정책을 **우회**할 수 있다.
...
이때 프록시와 API 서버는 다른 Origin 이지만 서버-서버 통신이기 때문에 CORS의 영향 없이 정상적으로 통신할 수 있고, API서버의 응답을 수신한 프록시 서버가 브라우저에게 응답을 전달하면 브라우저 입장에서는 동일한 Origin(`http://localhost:3000`)으로 인식하므로 API 서버와 자유로운 통신이 가능하다.
...
이는 쿠키의 속성 중 `SameSite` 설정과 관련이 있는데, 조건을 만족하지 않는 경우 별다른 메시지 없이 클라이언트의 쿠키가 전송되지 않지 때문에 문제 원인 파악에 어려움을 겪제 된다.
...
지금까지는 same-origin과 cross-origin만 구분했는데, `site`라는 용어가 등장했다. 어떠한 차이가 있는지 알아 보자.
...
`eTLD+N`은 eTLD로부터 N단계 앞의 부분까지 포함한 값을 의미한다.  
다음과 같은 도메인이 있다고 하자.
```http
api.example.org
```
...
### SameSite=None의 위험성
보안 설정을 강제로 비활성화하는 것은 항상 위험한 행동이다. SameSite 설정 때문에 쿠키가 설정되지 않는다고 해서 개발 시 `None` 값을 설정하는 경우가 있다. 그러한 경우 발생할 수 있는 위험성에 대해 알아보자.
...
`SameSite=None` 설정은 해당 쿠키를 마치 서드파티 쿠키처럼 관리하라는 설정이다. 모든 크로스 사이트 요청에 쿠키를 전송할 수 있게 되어 CSRF 공격에 취약해지는 것은 물론, 사용자의 브라우징 활동을 추적하는 데 악용될 여지도 있다.
...
이러한 경우, 로컬 환경의 host 설정을 통해 임시로 프론트엔드와 백엔드를 SameSite로 만들어 주면 `SameSite=None` 설정 없이 쿠키 차단을 우회할 수 있다.
...
브라우저의 개발자 도구 등을 통해 요청 헤더를 조회하여 현재 요청이 어떤 Origin과 Site로 평가되어 전송되는지를 확인할 수 있다.
